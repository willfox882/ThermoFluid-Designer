...
class TestFluidProps:
    ...
    def test_friction_factor_laminar(self):
        Re = 1000.0
        f = friction_factor(Re, relative_roughness=0.01)
    ...
    def test_friction_factor_turbulent_smooth(self):
        ...
        f = friction_factor(1e6, relative_roughness=0.0)
        assert 0.010 < f < 0.014

    def test_friction_factor_turbulent_rough(self):
        # High Re + high roughness → fully rough regime f > smooth
        f_rough  = friction_factor(1e6, relative_roughness=0.05)
        f_smooth = friction_factor(1e6, relative_roughness=0.0)
        assert f_rough > f_smooth

    def test_friction_factor_continuity_at_transition(self):
        # f must be continuous across Re=2300 and Re=4000 boundaries
        Re_vals = [2299.9, 2300.0, 2300.1, 3999.9, 4000.0, 4000.1]
        fs = [friction_factor(Re, 0.001) for Re in Re_vals]
        for i in range(1, len(fs)):
            assert abs(fs[i] - fs[i-1]) < 0.005, \
                f"Discontinuity at Re≈{Re_vals[i]:.1f}: f={fs[i-1]:.5f}→{fs[i]:.5f}"
    ...
    def test_zero_flow_edge_case(self):
        f = friction_factor(0.0, 0.001)  # should not raise
        assert f > 0
...
class TestPipe:
    ...
    def test_validate_bad_length(self):
        p = Pipe('P3', diameter=0.1, length=0.0)
        errs = p.validate()
